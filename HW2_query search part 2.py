''' Sidharth Sudhakar Nandury
    Python Programming HW - 2 Part 2
    Query Search
    Due date: 9/26/2014
'''
...
if ("and" in query) or (len(query)>1 and ("and" not in query)and("or" not in query)):
    if "and" in query:
        query.remove("and")
    if "or" in query:
        query.remove("or")
    print ("Performing AND search for: ",query)
    list_dict1=dict[query[0]]
    for a in query[1:]:
        list_dict2=dict[a]
        list_dict1=list(set(list_dict1).intersection(list_dict2))
    for thing in list_dict1:
        print("Found at: ", thing, data_list[thing][:50])
    dt2= datetime.now()
    print("Execution time:", dt2.microsecond-dt1.microsecond)


elif ("or" in query) and ("and" not in query): #"OR" search
    print("Performing OR search for: ", query)
    query.remove("or")
    for item in query:
        query_list=[]
        query_list=query_list+dict[item]
        query_list=list(set(query_list))
        for item in query_list:
            print("Found: ", item, data_list[item][:50])
    dt2= datetime.now()
    print("Execution time:", dt2.microsecond-dt1.microsecond)
    
elif (len(query)==1):
    for value in dict[query[0]]:
        print ("Found at: ", value, data_list[value][:50])
    dt2= datetime.now()
    print("Execution time:", dt2.microsecond-dt1.microsecond)
...
'''   
        ...
        #removes repeated items from the list and converting back to list from set
query.sort()                     #sorts the list in alphabetical order


if ("or" in query) and ("and" not in query):    #if "and" is present, "and" operation takes over from elif statement
    query.remove("or")                          #get rid of the "or" content from the list
    print("Performing OR search for: ", query)
    for i, quote in enumerate(data_list):
        for item in query:                      #useful if user uses multiple keywords
            if (item in quote):
...
else: #else if user enters just one string
